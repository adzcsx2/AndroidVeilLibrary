...
class VeilRecyclerFrameView : RelativeLayout {
    ...
    private val defaultItemCount = 10
    ...
    private fun getAttrs(attrs: AttributeSet?) {
        ...
        try {
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_veiled)) isVeiled =
            ...
            if (a.hasValue(R.styleable.VeilLayout_veilLayout_drawable)) drawable =
                a.getDrawable(R.styleable.VeilRecyclerFrameView_veilFrame_drawable)
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_radius)) radius =
                a.getDimension(R.styleable.VeilRecyclerFrameView_veilFrame_radius, radius)
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_shimmerEnable)) shimmerEnable =
                a.getBoolean(
                    R.styleable.VeilRecyclerFrameView_veilFrame_shimmerEnable, shimmerEnable
                )
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_baseColor)) baseColor =
                a.getColor(R.styleable.VeilRecyclerFrameView_veilFrame_baseColor, baseColor)
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_highlightColor)) highlightColor =
                a.getColor(
                    R.styleable.VeilRecyclerFrameView_veilFrame_highlightColor, highlightColor
                )
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_baseAlpha)) baseAlpha =
                a.getFloat(R.styleable.VeilRecyclerFrameView_veilFrame_baseAlpha, baseAlpha)
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_highlightAlpha)) highlightAlpha =
                a.getFloat(
                    R.styleable.VeilRecyclerFrameView_veilFrame_highlightAlpha, highlightAlpha
                )
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_dropOff)) dropOff =
                a.getFloat(R.styleable.VeilRecyclerFrameView_veilFrame_dropOff, dropOff)
            if (a.hasValue(R.styleable.VeilRecyclerFrameView_veilFrame_defaultChildVisible)) defaultChildVisible =
                a.getBoolean(
    ...
    /** Sets mask layout. */
    fun setVeilLayout(@LayoutRes layout: Int, size: Int = defaultItemCount) {
    ...
    fun setLayoutManager(layoutManager: RecyclerView.LayoutManager) {
        ...
        when (layoutManager) {
            is GridLayoutManager -> this.veiledRecyclerView.layoutManager =
                GridLayoutManager(context, layoutManager.spanCount)
            is StaggeredGridLayoutManager -> this.veiledRecyclerView.layoutManager =
                StaggeredGridLayoutManager(layoutManager.spanCount, layoutManager.orientation)
            is LinearLayoutManager -> this.veiledRecyclerView.layoutManager =
